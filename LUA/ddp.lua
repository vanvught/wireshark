...
local data = ProtoField.uint8("ddp.data.data", "Data", base.HEX)

ddp_proto.fields = { 
	flags1_version, flags1_push, flags1_query, flags1_reply, flags1_storage, flags1_time,
	flags2, message_type, id, offset, len, json, data
}
...
function ddp_proto.dissector(buffer, pinfo, tree)
  length = buffer:len()
	...
	if buffer(10,1):uint() == 0x7b then
		local messagetree = tree:add(ddp_proto,buffer(10),id_text)
		messagetree:add(json, buffer(10))
	else
		local data_size = buffer(8,2):uint()
		local messagetree = subtree:add(ddp_proto,buffer(10), "Data")
		messagetree:add(buffer(10, data_size), buffer(10,data_size):bytes():tohex())
	end
end
